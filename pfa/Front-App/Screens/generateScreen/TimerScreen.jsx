...
export default function TimerScreen() {
  ...
  const handleDayPress = (day) => {
    ...
    let newDates = { ...selectedDates.markedDates };
  
    if (!startDate || (startDate && endDate)) {
      // Start a new selection
      newDates = {
        [day.dateString]: { startingDay: true,Day: true, color: 'blue', textColor: 'white' }
      };
      setSelectedDates({ startDate: day.dateString, endDate: null, markedDates: newDates });
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        ...
        if (key === startDate || key === endDate) {
          newDates[key] = { startingDay: true, endingDay: true, color: 'blue', textColor: 'white' };
        } else {
          newDates[key] = { selected: true, color: 'blue', textColor: 'white' };
        }
      }
  
      setSelectedDates({ ...selectedDates, endDate: day.dateString, markedDates: newDates });
    }
  };
  ...
  function handleDone() {
    ...
    if (startDate && endDate) {
      // Here you can save the selected period or perform any other actions
